...
def calculate_clustering_delivery_time(cluster_centers, locations, labels, drone_speed, truck_speed, service_time, max_drones_per_cluster=N_DRONES):
    ...
    # List to store the arrival times at each cluster center
    arrival_times = []
    current_time = 0  # Time starts at zero (truck leaves the depot)

    for i, center_idx in enumerate(tsp_route[1:-1]):
        # Calculate the time taken to travel from the last center to this one
        travel_time_to_cluster = truck_distances[i] / truck_speed
        current_time += travel_time_to_cluster
        arrival_times.append(round(current_time, 4))  # Store the current arrival time at this cluster, rounded to 2 decimal places
    ...
    # Return arrival times along with other outputs
    return total_time, truck_time, total_drone_time, total_service_time, total_utilization, arrival_times
...
def save_data_to_csv(filename, **data):
    # if the name is Opt_Arrival_time the save it in the folder Route 
...
def evaluate_performance(n_iterations, drone_speed=40.0, truck_speed=60.0, service_time=5.0, disruption_probability=0.1, disruption_type='road_closure', num_robustness_simulations=100):
    ...
    # Save All locations to CSV for each iteration
    # Save the optimized Centers to CSV for each iteration
    # Save the optimized Routes to CSV for each iteration
    # Save the optimized Arrival times to CSV for each iteration
...
def save_iter_data_to_csv(iteration, locations, labels, optimized_centers, opt_route, arrival_times, csv_file='all_iterations_data.csv'):
    ...
    # Save the DataFrame to the CSV file, overwriting any existing file
    csv_path = f'{folder_name}/{csv_file}'
    if iteration == 0:
        # If it's the first iteration, overwrite any previous file
        iteration_data.to_csv(csv_path, index=False, mode='w')  # 'w' for write mode (overwrite)
    else:
        # Append to the file in subsequent iterations
