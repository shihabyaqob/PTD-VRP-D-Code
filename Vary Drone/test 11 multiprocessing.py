...
# Note: Remove the random seed initialization to ensure randomness across processes
...
def cluster_locations(locations, max_clusters):
...
def solve_tsp_with_gurobi(data):
    ...
    with gp.Env(empty=True) as env:
        ...
        with gp.Model(env=env) as model:
            # Adjust Gurobi parameters for multiprocessing
            model.setParam('Threads', 1)      # Use 1 thread per process
...
def get_truck_speed_at_time(delivery_time):
    try:
        speed_df = pd.read_csv('PS/Predicted_Speeds.csv')
...
def calculate_clustering_delivery_time(cluster_centers, locations, labels, drone_speed, truck_speed, service_time, max_drones_per_cluster):
    ...
    current_time = 8.0  # Start simulation at 8 AM (or adjust as needed)
...
def calculate_optimize_weights_delivery_time(optimized_weights, cluster_centers, locations, labels, drone_speed, truck_speed, service_time, num_drones):
    ...
    return calculate_clustering_delivery_time(optimized_centers, locations, labels, drone_speed, truck_speed, service_time,num_drones)
...
def optimize_weights(cluster_centers, locations, labels, drone_speed, truck_speed, service_time, num_drones):
    ...
    def objective(weights):
        ...
        delivery_time = calculate_optimize_weights_delivery_time(weights, cluster_centers, locations, labels, drone_speed, truck_speed, service_time, num_drones)[0]
...
def run_iteration(args):
    """Function to run a single iteration, used for multiprocessing."""
    (i, delivery_locations, drone_speed, truck_speed, disruption_probability, service_time, disruption_type, num_robustness_simulations, num_drones) = args

    # Re-seed the random number generator for each process
    np.random.seed(int(time.time()) % (2**32 - 1) + os.getpid())

    print("Iteration:", i)
    locations = generate_random_locations(delivery_locations, DEPOT)
    labels, cluster_centers = cluster_locations(locations, max_clusters=delivery_locations)
    num_clusters = len(np.unique(labels))

    # Initialize dictionaries to hold results
    results = {}

    results['Total_number_of_clusters'] = num_clusters
    ...
    if tsp_route:
        ...
        results['Total_delivery_times_tsp'] = tsp_time
        results['Total_travel_times_tsp'] = tsp_truck_time
        results['Total_truck_times_tsp'] = tsp_truck_time
        results['Total_service_times_tsp'] = tsp_service_time
        ...
        results['Total_robustness_tsp'] = (mean_adjusted_time_tsp, std_adjusted_time_tsp)

    cluster_time, cluster_truck_time, cluster_drone_time, cluster_service_time, drone_utilizations, _ = calculate_clustering_delivery_time(cluster_centers, locations, labels, drone_speed, truck_speed, service_time, num_drones)
    if cluster_time:
        results['Total_delivery_times_cluster'] = cluster_time
        results['Total_travel_times_cluster'] = cluster_truck_time + cluster_drone_time
        results['Total_truck_times_cluster'] = cluster_truck_time
        results['Total_drone_times_cluster'] = cluster_drone_time
        results['Total_service_times_cluster'] = cluster_service_time
        results['Total_drone_utilization_cluster'] = drone_utilizations
        ...
        results['Total_robustness_cluster'] = (mean_adjusted_time_cluster, std_adjusted_time_cluster)

    optimized_weights = optimize_weights(cluster_centers, locations, labels, drone_speed, truck_speed, service_time, num_drones)
    if optimized_weights is not None:
        optimize_time, optimize_truck_time, optimize_drone_time, optimize_service_time, drone_utilizations_opt, arrival_times_opt = calculate_optimize_weights_delivery_time(optimized_weights, cluster_centers, locations, labels, drone_speed, truck_speed, service_time, num_drones)
        results['Total_delivery_times_optimize'] = optimize_time
        results['Total_travel_times_optimize'] = optimize_truck_time + optimize_drone_time
        results['Total_truck_times_optimize'] = optimize_truck_time
        results['Total_drone_times_optimize'] = optimize_drone_time
        results['Total_service_times_optimize'] = optimize_service_time
        results['Total_drone_utilization_optimize'] = drone_utilizations_opt
        results['Optimize_arrival_times'] = arrival_times_opt
        ...
        results['Total_robustness_optimize'] = (mean_adjusted_time_optimize, std_adjusted_time_optimize)

    return results

def evaluate_performance(n_iterations, delivery_locations, drone_speed, truck_speed, disruption_probability, service_time, disruption_type, num_robustness_simulations, num_drones):
    ...
    print("Number of Delivery Locations:", delivery_locations)

    # Prepare arguments for each iteration
    args_list = [(i, delivery_locations, drone_speed, truck_speed, disruption_probability, service_time, disruption_type, num_robustness_simulations, num_drones) for i in range(1, n_iterations + 1)]

    # Determine the number of processors
    num_processors = 32  # Use all 32 processors
    print(f"Utilizing {num_processors} processors.")

    # Run in parallel
    with multiprocessing.Pool(processes=num_processors) as pool:
        results_list = pool.map(run_iteration, args_list)

    for results in results_list:
        if 'Total_number_of_clusters' in results:
            Total_number_of_clusters.append(results['Total_number_of_clusters'])
        if 'Total_delivery_times_tsp' in results:
            Total_delivery_times_tsp.append(results['Total_delivery_times_tsp'])
            Total_travel_times_tsp.append(results['Total_travel_times_tsp'])
            Total_truck_times_tsp.append(results['Total_truck_times_tsp'])
            Total_service_times_tsp.append(results['Total_service_times_tsp'])
            Total_robustness_tsp.append(results['Total_robustness_tsp'])
        if 'Total_delivery_times_cluster' in results:
            Total_delivery_times_cluster.append(results['Total_delivery_times_cluster'])
            Total_travel_times_cluster.append(results['Total_travel_times_cluster'])
            Total_truck_times_cluster.append(results['Total_truck_times_cluster'])
            Total_drone_times_cluster.append(results['Total_drone_times_cluster'])
            Total_service_times_cluster.append(results['Total_service_times_cluster'])
            Total_drone_utilization_cluster.append(results['Total_drone_utilization_cluster'])
            Total_robustness_cluster.append(results['Total_robustness_cluster'])
        if 'Total_delivery_times_optimize' in results:
            Total_delivery_times_optimize.append(results['Total_delivery_times_optimize'])
            Total_travel_times_optimize.append(results['Total_travel_times_optimize'])
            Total_truck_times_optimize.append(results['Total_truck_times_optimize'])
            Total_drone_times_optimize.append(results['Total_drone_times_optimize'])
            Total_service_times_optimize.append(results['Total_service_times_optimize'])
            Total_drone_utilization_optimize.append(results['Total_drone_utilization_optimize'])
            Optimize_arrival_times.append(results['Optimize_arrival_times'])
            Total_robustness_optimize.append(results['Total_robustness_optimize'])

    # After collecting all data, proceed to save and print statistics
    save_data_to_csv('Output.csv', Iteration=np.arange(1, n_iterations + 1),
                    ...
                    Total_drone_times_optimize=Total_drone_times_optimize)  # Save optimized arrival times in CSV
...
def save_data_to_csv(filename, **data):
    ...
    hypothesis_test_folder = ''
    full_path = os.path.join(hypothesis_test_folder, filename)
    
    if os.path.exists(full_path):
        filename = filename.split('.')[0] + '_2.csv'
        while os.path.exists(os.path.join(hypothesis_test_folder, filename)):
            filename = filename.split('_')[0] + '_' + str(int(filename.split('_')[1].split('.')[0]) + 2) + '.csv'
        full_path = os.path.join(hypothesis_test_folder, filename)
...
def main():
    # Define the number of delivery locations
    N_DELIVERY_LOCATIONS = 50  # Fixed number of delivery locations 
    
    # Iterate through drone counts from 2 to 16 with a step of 2
    for num_drones in range(12, 17, 2):
        print(f"\nRunning simulation with {N_DELIVERY_LOCATIONS} delivery locations and {num_drones} drones.")
        
        # Set the number of drones dynamically
        global N_DRONES
        N_DRONES = num_drones
        
        # Run the performance evaluation with the current number of drones
        evaluate_performance(n_iterations=30, 
                             delivery_locations=N_DELIVERY_LOCATIONS, 
