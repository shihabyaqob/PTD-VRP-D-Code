...
def optimize_weights(cluster_centers, locations, labels, drone_speed, truck_speed, service_time):
    ...
    def objective(weights):
        ...
        delivery_time = calculate_optimize_weights_delivery_time(weights, cluster_centers, locations, labels, drone_speed, truck_speed, service_time)[0]
...
def run_iteration(args):
    ...
    (i, delivery_locations, drone_speed, truck_speed, disruption_probability, service_time, disruption_type, num_robustness_simulations) = args
...
def evaluate_performance(n_iterations, delivery_locations, drone_speed, truck_speed, disruption_probability, service_time, disruption_type, num_robustness_simulations):
    ...
    args_list = [(i, delivery_locations, drone_speed, truck_speed, disruption_probability, service_time, disruption_type, num_robustness_simulations) for i in range(n_iterations)]
...
def save_data_to_csv(filename, **data):
    ...
    hypothesis_test_folder = 'Hypothesis_test/'
    full_path = os.path.join(hypothesis_test_folder, filename)
    
    if os.path.exists(full_path):
        filename = filename.split('.')[0] + '_10.csv'
        while os.path.exists(os.path.join(hypothesis_test_folder, filename)):
            filename = filename.split('_')[0] + '_' + str(int(filename.split('_')[1].split('.')[0]) + 10) + '.csv'
        full_path = os.path.join(hypothesis_test_folder, filename)
...
def main():
    for i in range(10):
        global N_DELIVERY_LOCATIONS
        N_DELIVERY_LOCATIONS =10 + i * 10
        print(f"Running simulation with {N_DELIVERY_LOCATIONS} delivery locations.")
        evaluate_performance(n_iterations=10, delivery_locations=N_DELIVERY_LOCATIONS, drone_speed=DRONE_SPEED, truck_speed=DEFAULT_TRUCK_SPEED, disruption_probability=0.1, service_time=SERVICE_TIME, disruption_type='road_closure', num_robustness_simulations=100)
