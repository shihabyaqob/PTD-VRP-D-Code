...
# Time-Dependent Speed Profiles for Truck
TIME_WINDOWS = [
...
def calculate_time_segment(travel_distance, current_time, truck_speed, time_window_end):
    """Calculate the time to travel within a single time window."""
    remaining_time_in_window = time_window_end - current_time
    travel_time_at_current_speed = travel_distance / truck_speed  # Time needed at current speed
    # If the truck can finish the trip within this time window
    if travel_time_at_current_speed <= remaining_time_in_window:
        return travel_time_at_current_speed, travel_distance
    else:
        # Calculate how far the truck can travel within this time window
        distance_travelable_in_window = truck_speed * remaining_time_in_window
        return remaining_time_in_window, distance_travelable_in_window

def get_next_time_window(current_time):
    """Get the next time window's start and end times, and speed."""
    for start, end, speed in TIME_WINDOWS:
        if start <= current_time % 24 < end:
...
def calculate_clustering_delivery_time(cluster_centers, locations, labels, drone_speed, truck_speed, service_time, max_drones_per_cluster=N_DRONES):
    ...
    for i, center_idx in enumerate(tsp_route[1:-1]):
        ...
        while remaining_distance > 0:
            start, end, truck_speed = get_next_time_window(current_time)  # Get speed and end time of the current window
            travel_time, distance_travelable = calculate_time_segment(remaining_distance, current_time, truck_speed, end)
            segment_travel_time += travel_time
            remaining_distance -= distance_travelable
