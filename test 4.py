...
def evaluate_performance(n_iterations, drone_speed=40.0, truck_speed=60.0, service_time=5.0, disruption_probability=0.1, disruption_type='road_closure', num_robustness_simulations=100):
    ...
    for _ in range(n_iterations):
        ...
        if cluster_time:
            ...
            cluster_route = solve_tsp(create_data_model(np.vstack([DEPOT, cluster_centers]), metric='cityblock'))
        ...
        if optimized_weights is not None:
            ...
            optimize_route = solve_tsp(create_data_model(np.vstack([DEPOT, optimized_centers]), metric='cityblock'))
